...
const NextMeetingCard = ({ meeting }: { meeting: Event | null }) => (
  <div className="p-6 bg-white rounded-xl shadow-sm border border-neutral-200">
    ...
    {meeting ? (
      <div>
        <span className={`inline-block px-3 py-1 text-sm font-semibold rounded-full mb-2 ${meeting.type === 'onsite' ? 'bg-info/10 text-info' : 'bg-success/10 text-success'
          }`}>
...
export default function DashboardPage() {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    // --- URLs to fetch ---
    const usersUrl = '/user/all';
    const attendanceUrl = '/attendance';
    const eventsUrl = '/event';
    // --- NEW Endpoints ---
    const latestAnnouncementUrl = '/announcements/latest';
    const popularForumUrl = '/forums/popular';
    ...
    try {
      ...
      const baseRequests = [
        api.get(eventsUrl),
        api.get(latestAnnouncementUrl),
        api.get(popularForumUrl),
      ];

      // Conditionally add admin-only fetches
      const isAdminRole = [1, 3, 4].includes(roleId);
      if (isAdminRole) {
        baseRequests.push(api.get(usersUrl));
        baseRequests.push(api.get(attendanceUrl));
      }
      ...
      if (eventsData.status === 200 && Array.isArray(eventsData.events)) {
        ...
        const sortedEvents = eventsData.events
          ...
          .sort((a: Event & { dateTime: Date }, b: Event & { dateTime: Date }) => a.dateTime.getTime() - b.dateTime.getTime());
        const upcoming = sortedEvents.find((e: Event & { dateTime: Date }) => e.dateTime > new Date());
